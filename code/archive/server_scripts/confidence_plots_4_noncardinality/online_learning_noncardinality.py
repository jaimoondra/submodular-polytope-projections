...
def online_mirror_descent_permutahedron(P: SubmodularPolytope, T: int, epsilon: float, seed):
    """
...
for seed in range(6, 21):
    ...
    times.to_csv('times_noncardinality_' + str(seed) + '.csv')
    iterates.to_csv('iterates_noncardinality_' + str(seed) + '.csv')
    regrets.to_csv('regrets_noncardinality_' + str(seed) + '.csv')
