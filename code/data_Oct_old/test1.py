...
def adaptive_AFW_cardinality_polytope(x, S, P, epsilon, func, grad_f, f_tol, time_tol,
    ...
    def update_S(S, gamma, Away, vertex):
        ...
        # todo: add check for vertices with weight less than 10^{-5}
...
def online_mirror_descent_permutahedron(P: Permutahedron, T: int, epsilon: float, seed, a, b):
    ...
    logging.warning('Starting PAV')
    _, _, regret_pav = \
        isotonic_projection_mirror_descent(n, P, T, loss_vectors_list, x_0, eta)

    print(regret_vanilla, regret_pav)


    return total_time_vanilla, time_steps_vanilla, fw_iterations_vanilla, regret_vanilla
...
P = Permutahedron(f)
for seed in range(1, outer):
    print('Run ', seed)
    t1, T1, i1, r1 = online_mirror_descent_permutahedron(P, T, epsilon, seed, a, b)
    print(t1, T1, i1, r1)
