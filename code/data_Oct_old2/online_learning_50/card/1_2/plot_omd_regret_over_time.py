...
T_u, T_a, T_c, T_d, T_o, T_i = [], [], [], [], [], []

for i in runs:
    regret_df = pd.read_csv('regrets_permutahedron50_1000_1_2_' + str(i) + '.csv', index_col=0,
                            header=None)
...
plt.fill_between(range(m), quantile25_i, quantile75_i, color='purple', alpha=0.1)
...
average_i_plot, = plt.plot(average_i, color='purple', linestyle='dotted', label='OMD-PAV')
