...
def generate_loss_functions_for_bipartite_submodular_function(P, a, b, seed, n: int, T: int):
    """
...
class SubmodularPolytope:
    ...
    def minimum_norm_point(self, eps: float):
        def get_base_vertex():
            print('n = ', n)
            return [self.f.function_value(set(range(i))) - self.f.function_value(set(range(i - 1)))
                    for i in range(n)]
...
def create_bipartite_graph(n: int, p: float, seed: int):
    """
    Function to create a random bipartite graph with bipartition (U, V), U = V = [n]
    :param n:
    :param p: probability of each edge existing
    :return: list of neighbors of each vertex in U
    """
    random.seed(seed)
    U_neighbors = []
    for u in range(n):
...
def vanilla_mirror_descent(n, epsilon, P, T, loss_vectors_list, x_0, eta):
    ...
    for t in range(T):
        ...
        # if t < 1:
        #     print('afw x =', x, y, np.linalg.norm(y - x))
...
def cut_optimized_mirror_descent(n, epsilon, P, T, loss_vectors_list, x_0, eta):
    ...
    for t in range(T):
        ...
        # if t < 1:
        #     print('adaptive afw x =', x, y, np.linalg.norm(y - x))
...
def online_mirror_descent_bipartite(P: SubmodularPolytope, T: int, epsilon: float, seed, a, b):
    """
    Performs online mirror descent on a a bipartite cardinality function
    :param P: permuathedron. See submodular_polytope.py for class definition
    :param T: number of iterations
    :param a: number of permutations
    :param b: swap distance of permutations
    :return: Total regret
    """
    n = len(P)
    ...
    b = b // 2
    loss_vectors_list = generate_loss_functions_for_bipartite_submodular_function(P, a, b, seed,
                                                                                  n, T)
    ...
    return total_time_vanilla, total_time_active_set_optimized, total_time_cut_optimized, \
        total_time_doubly_optimized, total_time_ofw, \
        time_steps_vanilla, time_steps_active_set_optimized, time_steps_cut_optimized, \
        time_steps_doubly_optimized, time_steps_ofw, \
...
print('n =', n, 'T =', T, 'epsilon =', epsilon, 'a =', a, 'b =', b, 'p =', p)


U_neighbors = create_bipartite_graph(n, p, seed=0)
f = BiparititeGraphSubmodularFunction(n, U_neighbors)
P = SubmodularPolytope(f)
for seed in range(start, end):
    print('Run ', seed)
    t1, t2, t3, t4, t5, T1, T2, T3, T4, T5, i1, i2, i3, i4, r1, r2, r3, r4, r5 =\
        online_mirror_descent_bipartite(P, T, epsilon, seed, a, b)

    times = pd.DataFrame(columns=[T1, T2, T3, T4, T5])
    iterates = pd.DataFrame(columns=[i1, i2, i3, i4])
    regrets = pd.DataFrame(columns=[r1, r2, r3, r4, r5])
    times.to_csv('times_noncardinality' + str(n) + '_' + str(T) + '_' + str(a) + '_' + str(b) +
                 '_' + str(seed) + '.csv')
    iterates.to_csv('iterates_noncardinality' + str(n) + '_' + str(T) + '_' + str(a) + '_' +
                    str(b) + '_' + str(seed) + '.csv')
    regrets.to_csv('regrets_noncardinality' + str(n) + '_' + str(T) + '_' + str(a) + '_' + str(b) +
                   '_' + str(seed) + '.csv')

    sourceFile = open('noncardinality1_' + str(n) + '_' + str(T) + '_' + str(a) + '_' + str(b) +
                      '.txt', 'a')
