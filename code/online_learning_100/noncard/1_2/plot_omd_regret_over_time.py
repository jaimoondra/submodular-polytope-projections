...
for i in runs:      # For each run of the experiment
    # Extract times
    regret_df = pd.read_csv('regrets_' + filename_prefix + str(i) + '.csv', index_col=0,
                            header=None)
    regret_df.index = range(k)                    # Reindex dataframe
    regret_df = regret_df.loc[variant_indices]       # Extract values only for the required runs
    regret_df = (normalized_value * regret_df) / regret_df.sum(axis=1)[0] # Normalize UAFW values to 1000
    for j in variant_indices:                          # Save these times to T for each variant
        T[j].append(list(regret_df.loc[j]))

for j in variant_indices:                          # For each variant
    T[j] = pd.DataFrame(T[j])               # Convert to pandas dataframe; easier to use than array
    T[j] = T[j].cumsum(axis=1).to_numpy()              # Convert T to have cumulative times over the
    # iterations
...
plt.title(r'Regret-over-time for OMD-AFW variants')          # Plot title
...
plt.legend(loc='upper right', fontsize=15)      # Location and size of legend
plt.savefig('regret_over_time.png', dpi=300)             # Figure name
plt.show()                                      # Show plot
